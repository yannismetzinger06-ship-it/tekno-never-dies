 ...
 *
 * startStars(canvas, config, spiralConfig) renvoie { renderer, scene, tick(dt) }
 * ou null si WebGL n'est pas disponible.
 *
 * Si spiralConfig est donné (réglages de la spirale), les étoiles ne passent
 * jamais sur la spirale : elles glissent autour quand leur trajet la croise.
 */
 
// Espace laissé entre les étoiles et la spirale (plus grand = plus loin).
const MARGE_SPIRALE = 0.25;
 
export function startStars(canvas, cfg, spiralCfg = null) {
  ...
  const stars = cfg.stars.map((spec) => {
    ...
    return {
      spec, mesh,
      radius: cfg.shapes[spec.shape].outer,   // taille de l'étoile (pour éviter la spirale)
      pos: null,                               // position réelle, après contournement
  ...
  // --- zone à éviter : la spirale -----------------------------------------
  // La spirale est dans un autre calque, avec sa propre caméra et plus loin
  // derrière. On calcule où elle tombe à l'écran pour la ramener à la
  // profondeur de chaque étoile.
  const tanStars = Math.tan(THREE.MathUtils.degToRad(fov / 2));
  let avoid = null;
  if (spiralCfg) {
    const sc = spiralCfg;
    avoid = {
      x: sc.position.x,
      y: sc.position.y,
      r: Math.max(sc.shape.startRadius, sc.shape.endRadius) + sc.shape.tubeRadius,
      depth: (sc.camera.distance - sc.position.z) * Math.tan(THREE.MathUtils.degToRad(sc.camera.fov / 2))
    };
  }
 
  // --- cadrage adaptatif ---------------------------------------------------
  let halfW = 5, halfH = 3.6, unit = 1, spiralUnit = 1;
  function layout() {
    ...
    // même calcul de taille que dans spiral.js
    if (spiralCfg) spiralUnit = THREE.MathUtils.clamp(portrait ? 0.75 : 1, 0.6, 1.3) * spiralCfg.scale;
  }
  layout();
  ...
  function tick(dt) {
    ...
    stars.forEach((s, i) => {
      ...
      let x = ax * (0.64 * Math.sin(s.wx[0] * k * t + s.phase[0]) + 0.36 * Math.sin(s.wx[1] * k * t + s.phase[1]));
      let y = ay * (0.64 * Math.sin(s.wy[0] * k * t + s.phase[2]) + 0.36 * Math.sin(s.wy[1] * k * t + s.phase[3]));
      const z = s.spec.z + 0.35 * Math.sin(s.wz * t + s.phase[4]);
 
      // contournement de la spirale
      if (avoid) {
        if (!s.pos) s.pos = { x, y };
        // l'étoile suit sa trajectoire en douceur...
        s.pos.x += (x - s.pos.x) * follow;
        s.pos.y += (y - s.pos.y) * follow;
        // ...mais n'entre jamais dans le cercle de la spirale : elle glisse sur le bord
        const f = ((distance - z) * tanStars) / avoid.depth;
        const cx = avoid.x * f, cy = avoid.y * f;
        const R = avoid.r * spiralUnit * f + s.radius * s.spec.scale * unit + MARGE_SPIRALE;
        let dx = s.pos.x - cx, dy = s.pos.y - cy;
